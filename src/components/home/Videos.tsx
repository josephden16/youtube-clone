...
const Videos = ({ videos, loading, error, retry }) => {
  ...
  const loadingSkeletonArray = Array(numberOfLoadingSkeletons)
    .fill(null)
    .map((_, index) => index);
    ...
    return (
      <section className="flex flex-wrap space-y-12 sm:space-y-0 sm:grid sm:gap-5 sm:grid-cols-3 lg:grid lg:grid-cols-3 xl:grid-cols-4 lg:gap-8 w-full">
        {loadingSkeletonArray &&
          loadingSkeletonArray.map((item: number) => (
  ...
  if (error) {
    return (
      <div className="text-center flex flex-col w-full h-full border my-auto justify-center items-center">
        <div>
          <img
            draggable={false}
            src={ErrorImage}
  ...
  return (
    <div className="text-center flex flex-col w-full h-full border my-auto justify-center items-center">
      ...
      <div className="font-semibold text-lightGray">Failed to fetch videos</div>
      <button
        style={{ outline: "0px" }}
        onClick={retry}
        className="font-semibold py-2 px-3 mt-6 transition-all duration-300 focus:bg-dark2 focus:text-white outline-none rounded-md"
