      ...
      <a href={`/watch?v=${video.id}`}>
        <div className="text-right static">
          <img
            src={video.posterURL}
            alt="cover"
            className="video-poster object-center rounded-3xl hover:opacity-80 transition-opacity duration-300 cursor-pointer w-full"
          />
          <span className="text-xs relative right-3 bottom-8 bg-gray  opacity-80 text-white pt-1 pb-1 pl-2 pr-2 rounded-xl">
            {formatVideoTime(parseInt(video.duration, 10))}
          </span>
        </div>
      </a>
      <div className="transition-colors ml-2 mr-2">
        <h3 className="font-bold capitalize text-sm -mt-4">
        ...
        <div className="text-black dark:text-lightGray text-xs flex justify-between">
          <div className="space-x-2">
            ...
            <span>
              {video.timeUploaded
                ? formatTime(video.timeUploaded.seconds)
                : time}
            </span>
          </div>
          <div>
