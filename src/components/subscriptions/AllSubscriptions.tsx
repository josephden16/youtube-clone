...
export default function AllSubscriptions({ user }) {
  ...
  return (
    <div className="lg:-ml-3 w-full mt-5 lg:mt-3 mx-3">
          ...
          subscriptions.map((channel, index) => (
            <div key={index}>
              <Link
                ...
                className="flex lg:grid space-x-2 lg:grid-cols-1 lg:space-x-0 lg:place-items-center items-center space-y-1 hover:opacity-70"
              >
                <img
                  src={channel.channelPhotoURL}
                  className="w-12 lg:w-14 rounded-circle"
                  alt="channel"
                />
                <span className="text-base text-center lg:text-sm">
