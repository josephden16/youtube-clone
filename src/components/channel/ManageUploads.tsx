...
export default function ManageUploads({ videos, updateCurrentVideo, openDeleteModal }) {
  return (
    <div>
      ...
      <div className="flex flex-col space-y-8 md:space-y-0 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-8 xl:grid-cols-4 lg:space-y-0">
        {videos &&
          videos.map((video: any, index: number) => (
            <ManageVideo openDeleteModal={openDeleteModal} key={index} video={video} updateCurrentVideo={updateCurrentVideo} />
          ))}
